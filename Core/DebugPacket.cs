...
partial class Plugin
{
    ...
    {
        if (this.config.DebugPacket.Value.In.Value.Handle(self.readBuffer[start]))
        {
    ...
    {
        if (this.config.DebugPacket.Value.Out.Value.Handle(msgType))
        {
    ...
    {
        if (this.config.DebugPacket.Value.BytesOut.Value.Handle(args.Data[args.Offset + 2]))
        {
    ...
    private void MMHook_DebugPacket_CatchGet(On.Terraria.MessageBuffer.orig_GetData orig, MessageBuffer self, int start, int length, out int messageType)
    {
        ...
        {
            if (this.config.DebugPacket.Value.ShowCatchedException > Config.DebugPacketSettings.CatchedException.Uncommon)
            {
